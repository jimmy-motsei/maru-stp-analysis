...
async function main() {
  try {
    console.log('🚀 Step 1: Creating Notion database...\n');
    
    // Step 1: Create database with properties
    ...
    // Step 2: Wait for Notion to sync
    console.log('⏳ Step 2: Waiting for Notion to sync (5 seconds)...');
    await sleep(5000);
    
    // Step 3: Verify the database properties
    console.log('\n🔍 Step 3: Verifying database properties...');
    const dbInfo = await notion.databases.retrieve({ database_id: databaseId });
    const props = Object.keys((dbInfo as any).properties || {});
    console.log(`   Properties found: ${props.join(', ')}`);
    
    if (props.length < 5) {
      console.log('\n⚠️  Properties not fully synced. Trying alternative approach...');
      
      // Update properties explicitly
      console.log('   Adding properties via update...');
    ...
    // Step 4: Add tasks (using only the title first to verify)
    ...
    for (const task of tasks) {
      try {
        ...
        // If full properties fail, try with just the title
        try {
          await notion.pages.create({
            parent: { database_id: databaseId },
            properties: {
              'Name': {
                title: [{ text: { content: `${task.name} | ${task.phase} | ${task.priority}` } }],
              },
            },
          });
          console.log(`  ⚠️ ${task.name} (title only)`);
          successCount++;
        } catch (err2: any) {
          console.log(`  ❌ ${task.name}: ${err2.message}`);
        }
      }
      await sleep(400);
    }
    ...
    console.log('\n' + '='.repeat(60));
    console.log(`🎉 Created ${successCount} of ${tasks.length} tasks`);
    console.log('='.repeat(60));
    console.log(`\n📌 Add to .env.local:\n   NOTION_DATABASE_ID=${databaseId}`);
    console.log(`\n🔗 View: ${(createResponse as any).url}`);
