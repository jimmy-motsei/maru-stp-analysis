...
async function main() {
  try {
    console.log('🚀 Creating Notion database (fixed - no status type)...\n');
    
    // Create database with SELECT instead of STATUS (status type not supported via API!)
    const createResponse = await notion.databases.create({
      ...
      properties: {
        ...
        // Using SELECT instead of STATUS - Notion API doesn't support creating status properties!
        'Status': {
          select: {
            options: [
    ...
    console.log(`   URL: ${(createResponse as any).url}\n`);
    
    // Wait for Notion to sync
    console.log('⏳ Waiting for Notion to sync (3 seconds)...');
    await sleep(3000);
    
    // Verify the database properties
    console.log('\n🔍 Verifying database properties...');
    const dbInfo = await notion.databases.retrieve({ database_id: databaseId });
    const props = Object.keys((dbInfo as any).properties || {});
    console.log(`   ✅ Properties found: ${props.join(', ')}\n`);
    
    if (props.length < 5) {
      console.log('⚠️  Warning: Not all properties were created. Check Notion API permissions.');
    }
    ...
    console.log('\n' + '='.repeat(60));
    if (errorCount === 0) {
      console.log(`🎉 SUCCESS! All ${successCount} tasks created with full properties!`);
    } else {
      console.log(`Created ${successCount} of ${tasks.length} tasks (${errorCount} failed)`);
    }
    console.log('='.repeat(60));
    console.log(`\n📌 Update your .env.local with:\n   NOTION_DATABASE_ID=${databaseId}`);
    console.log(`\n🔗 View your database: ${(createResponse as any).url}`);
    console.log('\n💡 Tip: You can change the Status column to a proper "Status" type');
    console.log('   manually in Notion if you want the kanban-style status feature.');
